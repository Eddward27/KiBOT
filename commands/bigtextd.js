exports.run = (client, message, args, sender, perms) => {
    ...
    if (!args[0]) {
        message.channel.send(`Especifíca un texto para escribir \`${settings.prefix}bigtextd <texto>\``);
        return console.log(`${chalk.bgRed('[BigTextD]')} ${sender.username}@${message.channel.name} - Sin texto - [${moment().format('YYYY-MM-DD HH:mm:ss')}]`);
    }
    ...
    console.log(`${chalk.bgRed('[BigTextD]')} ${sender.username}@${message.channel.name} - ${textoConsola} - [${moment().format('YYYY-MM-DD HH:mm:ss')}]`);
};
...
exports.help = {
    name: 'bigtextd',
    description: 'Genera un texto con indicadores regionales y elimino tu mensaje',
    usage: 'bigtextd <texto>'
};
